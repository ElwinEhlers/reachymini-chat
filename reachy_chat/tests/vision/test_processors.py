...
from reachy_chat.vision.processors import (
    VisionConfig,
    VisionManager,
    VisionProcessor,
...
def mock_torch() -> Any:
    ...
    with patch("reachy_chat.vision.processors.torch") as mock:
...
def mock_transformers() -> Any:
    ...
    with patch("reachy_chat.vision.processors.AutoProcessor") as proc, \
         patch("reachy_chat.vision.processors.AutoModelForImageTextToText") as model:
...
def test_vision_processor_initialization_failure(mock_torch: Any) -> None:
    ...
    with patch("reachy_chat.vision.processors.AutoProcessor") as mock_proc:
...
def test_vision_processor_process_image_success(mock_torch: Any, mock_transformers: Any) -> None:
    ...
    with patch("reachy_chat.vision.processors.cv2") as mock_cv2:
...
def test_initialize_vision_manager_success(mock_torch: Any, mock_transformers: Any, mock_camera: Mock) -> None:
    ...
    with patch("reachy_chat.vision.processors.snapshot_download") as mock_download, \
         patch("reachy_chat.vision.processors.os.makedirs"), \
         patch("reachy_chat.vision.processors.config") as mock_config:
...
def test_initialize_vision_manager_processor_failure(mock_torch: Any, mock_camera: Mock) -> None:
    ...
    with patch("reachy_chat.vision.processors.snapshot_download"), \
         patch("reachy_chat.vision.processors.os.makedirs"), \
         patch("reachy_chat.vision.processors.config") as mock_config, \
         patch("reachy_chat.vision.processors.AutoProcessor") as mock_proc:
