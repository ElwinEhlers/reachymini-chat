...
from reachy_chat.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from reachy_chat.config import config  # noqa: F401
from reachy_chat.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from reachy_chat.tools.background_tool_manager import BackgroundToolManager


logger = logging.getLogger(__name__)


DEFAULT_PROFILES_MODULE = "reachy_chat.profiles"
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"reachy_chat.tools.{tool_name}"
            try:
                source = _try_load_tool(
                    tool_name,
